...
- 사설 연구 파이프라인(run-research 상당)을 GitHub Actions로 자동화(cron UTC 11:00 = KST 20:00).
  **repo는 PUBLIC 유지 · 외부 저장소 없음(GitHub-only)** → 안전선 엄격.
- **상태 지속 = GitHub Actions cache**(경량 ledger `data/research_private`; **papers.db 268MB는 cache
  제외**·별도 로컬 아티팩트). 손대체 불가한 **렉시콘은 Secret `LEXICON_SEED_B64`(base64, ~21KB)로 시딩**
  — cache 비면 부트스트랩 후 멘션은 공개 data/에서 `concept_extract --day`로 재파생. **CI는 증분 --day만**
  (대규모 --backfill 금지=로컬). **Secret 미설정이면 워크플로 no-op**(안전).
  상세·셋업·cold-start 검증 = `data/research_private/notes/CI_CACHE_SEED.md`.
- **안전선**: 사설 데이터 **비커밋**(cache/Secret에만) · 공개 커밋은 **사니타이즈 `research_stats.json`
  1파일만** · 개념-레벨 stdout은 **사설 로그로 리다이렉트**(PUBLIC Actions 로그에 개념명·본문 금지) ·
