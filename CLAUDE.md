...
- **상태 지속 = GitHub Actions cache**(사설 트리·papers.db). 손대체 불가한 **렉시콘은 Secret
  `LEXICON_SEED_B64`(base64, ~21KB)로 시딩** — cache 비면 부트스트랩 후 멘션은 공개 data/에서 재파생.
  **Secret 미설정이면 워크플로 no-op**(안전). 상세·셋업 = `data/research_private/notes/CI_MIGRATION.md`.
- **안전선**: 사설 데이터 **비커밋**(cache에만) · 공개 커밋은 **사니타이즈 `research_stats.json` 1파일만** ·
