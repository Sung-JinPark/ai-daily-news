...
// ---------------------------------------------------------------------------
// D-1: weekly movers — the PUBLIC counterpart of the private research layer's
// velocity metrics. Boundary contract (research/README.md): public pages may
// show coarse aggregates recomputable from the git-tracked aggregates at
// build time (raw counts, 7-day-window diffs). The paper-grade signal —
// EMA-smoothed daily velocity, acceleration, burst z-scores, community
// evolution — stays in the gitignored private layer until publication.
// Windows are rolling 7 days anchored at the LATEST DATA DAY (never wall
// clock) so the same commit always builds the same page.
// ---------------------------------------------------------------------------
...
/**
 * Entity mention counts, recent 7 days vs the 7 days before that,
 * anchored at the newest day in entity_mentions.jsonl. Returns the top
 * gainers and losers with explicit tiebreaks (delta, recent, entity) so
 * equal values can never reorder between builds. When the corpus spans
 * fewer than 14 calendar days both lists are empty and the caller
 * renders a not-enough-data note instead.
 */
export function entityWeeklyDelta(topN: number = 8): {
  asOf: string | null;
  spanDays: number;
  gainers: WeeklyDelta[];
  losers: WeeklyDelta[];
  ...
  const rows = loadEntityMentions();
  if (rows.length === 0) return { asOf: null, spanDays: 0, gainers: [], losers: [] };
  let minDay = rows[0].day;
  let maxDay = rows[0].day;
  for (const r of rows) {
  ...
  const spanDays =
    Math.round(
      (new Date(maxDay + "T00:00:00Z").getTime() - new Date(minDay + "T00:00:00Z").getTime()) /
        86400000,
    ) + 1;
  if (spanDays < 14) return { asOf: maxDay, spanDays, gainers: [], losers: [] };

  const recentStart = shiftDay(maxDay, -6); // inclusive 7-day window
  const priorStart = shiftDay(maxDay, -13);
  const acc = new Map<string, WeeklyDelta>();
  for (const r of rows) {
    if (r.day < priorStart || r.day > maxDay) continue;
    const key = `${r.entity_type}${r.entity}`;
    ...
    if (r.day >= recentStart) cur.recent += 1;
    else cur.prior += 1;
    acc.set(key, cur);
  }
  const all = Array.from(acc.values());
  for (const e of all) e.delta = e.recent - e.prior;
  const byGain = [...all].sort(
    (a, b) => b.delta - a.delta || b.recent - a.recent || a.entity.localeCompare(b.entity),
  );
  const byLoss = [...all].sort(
    (a, b) => a.delta - b.delta || b.prior - a.prior || a.entity.localeCompare(b.entity),
  );
  return {
    asOf: maxDay,
    spanDays,
    gainers: byGain.filter((e) => e.delta > 0).slice(0, topN),
    losers: byLoss.filter((e) => e.delta < 0).slice(0, topN),
  };
}
...
/**
 * Co-occurrence pair weights, recent 7 days vs prior 7 days, anchored
 * at the newest day in entity_cooccurrence.jsonl. Pair orientation is
 * canonicalized (a < b) the same way cooccurrenceGraph does. `isNew`
 * marks pairs with zero prior-window observations. Same <14-day guard
 * and tiebreak discipline as entityWeeklyDelta.
 */
export function cooccurrenceWeeklyDelta(topN: number = 10): {
  asOf: string | null;
  spanDays: number;
  pairs: PairDelta[];
} {
  const rows = loadEntityCooccurrence();
  if (rows.length === 0) return { asOf: null, spanDays: 0, pairs: [] };
  let minDay = rows[0].day;
  let maxDay = rows[0].day;
  for (const r of rows) {
    if (r.day < minDay) minDay = r.day;
  ...
  if (spanDays < 14) return { asOf: maxDay, spanDays, pairs: [] };

  const recentStart = shiftDay(maxDay, -6);
  const priorStart = shiftDay(maxDay, -13);
  const acc = new Map<string, PairDelta>();
  for (const r of rows) {
    if (r.day < priorStart || r.day > maxDay) continue;
    // Canonical orientation so (a,b) and (b,a) accumulate together.
    const swap = r.entity_a > r.entity_b;
    const a = swap ? r.entity_b : r.entity_a;
    const b = swap ? r.entity_a : r.entity_b;
    const a_type = swap ? r.entity_b_type : r.entity_a_type;
    const b_type = swap ? r.entity_a_type : r.entity_b_type;
    const key = `${a}${b}`;
    const cur = acc.get(key) ?? { a, a_type, b, b_type, recent: 0, prior: 0, delta: 0, isNew: false };
    if (r.day >= recentStart) cur.recent += 1;
    else cur.prior += 1;
    acc.set(key, cur);
  }
  const all = Array.from(acc.values());
  for (const p of all) {
    p.delta = p.recent - p.prior;
    p.isNew = p.prior === 0 && p.recent > 0;
  }
  const ranked = all
    .filter((p) => p.recent > 0)
    .sort(
  ...
  return { asOf: maxDay, spanDays, pairs: ranked.slice(0, topN) };
}
